...
for _, row in df[df["Device"].isin(selected_devices)].iterrows():
    is_babbage = row["Device"] == "Machine analytique de Babbage"
    is_eniac = row["Device"] == "ENIAC"
    fig.add_annotation(
        x=row["Year"],
        y=np.log10(row["CPS/$1"]),
        text=row["Device"],
        showarrow=False,
        yshift=0 if is_babbage or is_eniac else 15,
        xshift=5 if is_babbage else (-10 if is_eniac else 0),
        xanchor="left" if is_babbage else "right",
        font=dict(size=FONT_SIZE),
    )

# Customize the plot
fig.update_traces(marker=dict(size=10, opacity=0.8, color=SEA_BLUE))
...
fig.update_yaxes(dtick=3, tickmode="array", 
                 tickvals=[1e-6, 1e-3, 1e0, 1e3, 1e6, 1e9, 1e12],
                 ticktext=["10⁻⁶", "10⁻³", "10⁰", "10³", "10⁶", "10⁹", "10¹²"],
                 tickfont=dict(size=FONT_SIZE))
fig.update_xaxes(title="", tickfont=dict(size=FONT_SIZE))
# Show the plot
